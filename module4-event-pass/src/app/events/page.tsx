...
export default async function EventsPage({ searchParams }: EventsPageProps): Promise<React.ReactElement> {
  ...
  const validCategory = EVENT_CATEGORIES.includes(params.category as EventCategory)
    ? (params.category as EventCategory)
    : undefined;

  const validStatus = EVENT_STATUSES.includes(params.status as EventStatus)
    ? (params.status as EventStatus)
    : undefined;

  const validPriceMax = params.priceMax && !Number.isNaN(Number(params.priceMax))
